...
class TelegramBot:
                ...
                if messages:
                    for message in messages:
                        try:
                            ...
                            self.send_answer(chat_id, f"Calculating momentum portfolio for {port_date_begin.strftime('%B-%Y')}...")  
                            ...
                            cursor = conn.cursor()
                            query = "SELECT * FROM monthly_portfolios ORDER BY id DESC LIMIT 1"
                            ...
                            if last_row is None or last_row[1].strftime("%m-%Y") != port_date_begin.strftime("%m-%Y"):
                                ...
                                cursor = conn.cursor()
                                cursor.execute(
                                    "INSERT INTO monthly_portfolios (month, asset1, asset2, asset3, asset4, asset5) VALUES (%s, %s, %s, %s, %s, %s)", 
    ...
    def wrangle_stocks(self):
        '''
        This function is used to retrieve a list of stocks from a specific API endpoint (Brazilian stock exchange, B3).
