...
def cumret_plot(tickers_list, index):
    ...
    df = yf.download(tickers_list + [index_ticker[index.upper()]], start=start_date, end=end_date, auto_adjust=True)[["Open", "Close"]]
    ...
    ret_port = (np.cumprod(1 + df_ret[[value for value in tickers_list if value != index_ticker[index.upper()]]].mean(axis=1))-1)*100
    ret_index = (np.cumprod(1 + df_ret[index_ticker[index.upper()]])-1)*100
    ...
    sns.lineplot(data=ret_index, ax=ax, label=index.upper())
