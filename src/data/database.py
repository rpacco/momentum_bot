...
def last_row_db(connection, mensagem):
    ...
    query = f'''
            ...
            WHERE eq_index = '{mensagem.text.replace('/', '').upper()}'
            ORDER BY month DESC
            LIMIT 1;
            '''
...
def insert_data_db(first_day_month, conn, mensagem, momentum_stocks):
    with conn.cursor() as cursor:
        cursor.execute(
            ...
            (mensagem.text.replace('/', '').upper(), first_day_month, momentum_stocks[0], momentum_stocks[1], momentum_stocks[2], momentum_stocks[3], momentum_stocks[4])
        )
    conn.commit()
